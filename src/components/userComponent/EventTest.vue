...
&#125;)
      ...
      <h3>결과:</h3>
      <div>
        <button v-on:click="counter += 1">버튼을 클릭한 횟수는 {{ counter }} 번</button>
      </div>
      <h2 id="메소드-이벤트-핸들러"><a href="#메소드-이벤트-핸들러" class="headerlink" title="메소드 이벤트 핸들러"></a>메소드 이벤트 핸들러</h2><p>많은 이벤트 핸들러의 로직은 더 복잡할 것이므로, JavaScript를 <code>v-on</code> 속성 값으로 보관하는 것은 간단하지 않습니다. 이 때문에 <code>v-on</code>이 호출하고자 하는 메소드의 이름을 받는 이유입니다.</p>
        <p>예제:</p>
<pre>
<code class="hljs html"><span class="hljs-tag">&lt;<span class="hljs-name">div</span> <span class="hljs-attr">id</span>=<span class="hljs-string">"example-2"</span>&gt;</span>
...
<h3>결과:</h3>
        ...
        <pre>
<code class="hljs html"><span class="hljs-tag">&lt;<span class="hljs-name">button</span> <span class="hljs-attr">v-on:click</span>=<span class="hljs-string">"warn('Form cannot be submitted yet.',$event)"</span>&gt;</span>
  Submit
...
<span class="hljs-comment">&lt;!--즉, 내부 엘리먼트를 대상으로 하는 이벤트가 해당 엘리먼트에서</span>
<span class="hljs-comment">처리되기 전에 여기서 처리합니다.--&gt;</span>
...
<span class="hljs-comment">&lt;!--event.target이 엘리먼트 자체인 경우에만 트리거를 처리한다.--&gt;</span>
...
<pre><code class="hljs html"><span class="hljs-tag">&lt;<span class="hljs-name">input</span> <span class="hljs-attr">v-on:keyup.page-down</span>=<span class="hljs-string">"onPageDown"</span>&gt;</span></code></pre>
    ...
    <span class="hljs-tag">
&lt;<span class="hljs-name">input</span> @<span class="hljs-attr">keyup.alt.67</span>=<span class="hljs-string">"clear"</span>&gt;</span>
